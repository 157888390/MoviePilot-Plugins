...
服务端写标签的代码是 `tagger.artist = metadata.singer`（`music-tag-native`），**只认一个字符串**，
所以「许嵩、何曼婷」会被原样写成一个 artist 值。而 MoviePilot 解析时 `_music_string_list()`
对单值不做任何切分，于是它被当成**一个整体艺术家**，与 MusicBrainz 返回的
`['许嵩', '何曼婷']` 求不到交集 → 候选全部判为不匹配 → 拿不到 `media_source` / `media_id`
→ 分类事实构建失败 → **整理被拒**（日志：`媒体识别结果未匹配到媒体分类`）。

实测同一个文件、同一个文件名，只改 artist 标签的单值/多值：

| artist 标签 | 识别结果 | 整理 |
| --- | --- | --- |
| `['许嵩、何曼婷']` | `音乐 素颜`（无年份） | 失败 |
| `['许嵩', '何曼婷']` | `音乐 素颜 (2010)` | 成功 |
...
- 多歌手标签必须在 **rename 之前**写入：改名后文件立刻对外可见，目录监控可能马上触发整理，
  之后再写就是和监控抢时间。mutagen 按文件内容识别容器，所以 `.part` 这种无扩展名文件也能写。
